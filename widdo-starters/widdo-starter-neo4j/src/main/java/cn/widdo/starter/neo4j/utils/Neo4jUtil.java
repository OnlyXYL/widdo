...
public class Neo4jUtil {
		...
		for (org.neo4j.driver.Record resultL : resultLs) {
			...
			for (String key : keys) {
				try {
					...
					// 设置path
					switch (typeName) {
						case Neo4jType.NODE -> setValue(value, Neo4jType.NODE,
								Neo4jConvertor.build().convertNeo4jNodeToNode(valueOrigin.asNode()));
						case Neo4jType.RELATIONSHIP -> setValue(value, Neo4jType.RELATIONSHIP, Neo4jConvertor.build()
								.convertNeo4jRelationshipToRelationship(valueOrigin.asRelationship()));
						...
						case Neo4jType.ISODURATION -> setValue(value, Neo4jType.ISODURATION,
								convertor.convertNeo4jDurationToDuration(valueOrigin.asIsoDuration()));
	 ...
	 **/
	private static void pathResult(org.neo4j.driver.Value valueOrigin, Neo4jConvertor convertor,
		...
		pathOrigin.relationships()
				.forEach(r -> path.addRelationship(convertor.convertNeo4jRelationshipToRelationship(r)));
	 ...
	 **/
	private static void listResult(org.neo4j.driver.Value valueOrigin, Neo4jConvertor convertor,
			cn.widdo.starter.neo4j.entity.Value value) {
	 ...
	 **/
	private static void pointResult(org.neo4j.driver.Value valueOrigin, Neo4jConvertor convertor,
			cn.widdo.starter.neo4j.entity.Value value) {
	 ...
	 **/
	public static Result<?> toExecute(Driver driver, String model, String cql, Map<String, Object> map,
			CQLFunction cqlFun) {
		...
		try {
			...
			if (Neo4jConstants.RUNNER_READ.equals(model)) {
				return session.executeRead(tx -> cqlFun.execute(cql, map, tx));
			}
			return session.executeWrite(tx -> cqlFun.execute(cql, map, tx));
	...
	public static void printCQL(String cql) {
		final String format = String.format("---client ip:%s  cql: %s ...", NetUtil.getRealIp(),
				cql.substring(0, Math.min(cql.length(), 100)));
		LOG.info(format);
	}
