...
/**
 * delete procedure. using this, you can batch delete nodes and it`s relation ,which nodes
 * have relation.
 *
 * @author XYL
...
public class DeleteProcedure {
	 ...
	 * <p>
	 * note that: you don`t pass in your parameters as params parameter to the apoc call .
	 * and with 3.x forward using apoc in this way.
	 * <p>
	 * but with 4.4 and newer versions you can utilize the
	 * <code>call {} in transactions </code>,like this:
	 * <p>
	 * <code>
	 ...
	 * </code>
	 * <p>
	 * however, if node deleted hava a large of number relationships, it may still lead to
	 * out of heap errors. In this case it`s better to delete the relationships first and
	 * then the nodes in batches. Like this:
	 * <p>
	 * <code>
	 * <p>
	 ...
	 * </code>
	 * <p>
	 * you can get more from this {@link "<a href=
	 * "https://neo4j.com/developer/kb/large-delete-transaction-best-practices-in-neo4j/?_ga=2.113458306.465137194.1677635279-1264917599.1677635279"/>"}
	...
	@Description(
			value = "batch delete nodes and it`s relations, if the node has relations,return the count of deleted.")
	public Stream<DeleteContainer> delete(@Name("label") String label, @Name("ids") List<Long> ids,
		...
		final String cypher = String.format("""
				...
				{batchSize:%s,parallel:%s,iterateList:%s}) YIELD operations return operations""", labelCypher,
				matchWhere, labelCypher, batchSize, parallel, iterateList);
