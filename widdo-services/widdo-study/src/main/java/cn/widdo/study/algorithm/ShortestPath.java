...
public class ShortestPath {
	 ...
	 * 顶点.
	 */
	private static final char[] vertex = { 'A', 'B', 'C', 'D', 'E', 'F' };
	 ...
	 * 邻接矩阵.
	 */
	private static final int[][] matrix = new int[vertex.length][vertex.length];
	...
	public static class Dijkstra {
		...
		/**
		 * 参数adjMatrix:为图的权重矩阵，权值为-1的两个顶点表示不能直接相连 函数功能：返回顶点0到其它所有顶点的最短距离，其中顶点0到顶点0的最短距离为0.
		 * @param matrix
		 * @param source
		 * @author XYL
			...
			for (int i = 0; i < matrix.length; i++) {
				if (i != source) {
					if (shortest[i] == M) {
						...
						System.out.println(
								vertex[source] + "到" + vertex[i] + "的最短路径为：" + path[i] + "，最短距离是：" + shortest[i]);
					}
				}
			}
		}
